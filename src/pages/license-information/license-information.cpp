...
LicenseInformation::LicenseInformation(QWidget *parent)
    : QWidget(parent), m_activeGuide(NULL), m_licenseInfoWidget(NULL),
...
bool LicenseInformation::readLicenseInfo() {
  ...
  if (!InfoDbus::KylinLicense::getLicenseJson(licenseInfo)) {
      ...
      if (InfoDbus::KylinLicense::getServiceStatus(status)) {
        ...
        case LICENSE_SERVICE_STATUS_INVALID: {
          ui->lab_status->setText(
              (QString("%1 (%2: %3)"))
                  .arg(tr("The current time is illegal"))
        ...
        case LICENSE_SERVICE_STATUS_UNEXPIRED: {
          ...
          ui->lab_status->setText(
              QString("%1%2")
                  .arg(tr("Not activated. Trail expiration: "))
                  .arg(dueTime));
        }
        default:
          break;
    ...
    case LICENSE_STATUS_REGISTERD: {
      ...
      KLOG_INFO() << "due time = " << dueTime << "year: " << expiredTime.year()
      ...
      ui->btn_status->setStyleSheet(
          "QToolButton#btn_status{background: transparent;}");
      break;
    }
    default:
      break;
...
void LicenseInformation::onBtnStatusClicked() {
  if (!m_isActive) // popup Active Guide
  {
    ...
    connect(m_activeGuide, SIGNAL(systemIsActived(bool)), this,
            SLOT(updateLicenseInfo(bool)));
    m_activeGuide->raise();
    m_activeGuide->show();
  } else // popup informations
  {
    ...
    m_licenseInfoWidget->move(
        screenGeometry.x() +
            (screenGeometry.width() - m_licenseInfoWidget->width()) / 2,
        screenGeometry.y() +
            (screenGeometry.height() - m_licenseInfoWidget->height()) / 2);
...
void LicenseInformation::updateLicenseInfo(bool isregister) {
  ...
  if (isregister) {
    ...
    if (!InfoDbus::KylinLicense::getLicenseJson(licenseInfo)) {
      ...
      ui->btn_status->setIcon(QIcon(":/images/license_info.svg"));
      ui->btn_status->setStyleSheet(
          "QToolButton#btn_status{background: transparent;border: none;}");
      ui->btn_status->setFixedSize(QSize(16, 16));
...
QSize LicenseInformation::sizeHint() const { return QSize(600, 600); }
