...
// You may need to build the project (run Qt uic code generator) to get "ui_password-expiration-policy-page.h" resolved
...
PasswordExpirationPolicyPage::PasswordExpirationPolicyPage(QWidget *parent) : QWidget(parent), ui(new Ui::PasswordExpirationPolicyPage)
{
    ui->setupUi(this);
    initUI();
}
...
void PasswordExpirationPolicyPage::updateInfo()
{
    ...
    KLOG_DEBUG() << "password_expiration_policy" << policy;
    ...
    {
        KLOG_ERROR() << "password expiration policy format error!!";
        return;
    }
    ...
    KLOG_DEBUG() << "userExpirationTime" << userExpirationTime;
    KLOG_DEBUG() << "lastChangedTime" << lastChangedTime;
    KLOG_DEBUG() << "maxVaildDays" << maxVaildDays;
