...
void AdvanceSettings::setInfo (QString account, AdvanceSettingsInfo &info)
{
    if (!info.uid.isEmpty())
    {
        ui->check_specifyUserID->setChecked(true);
        ui->edit_userID->setText(info.uid);
    }
    else
    {
        ui->check_specifyUserID->setChecked(false);
    }

    if (!info.shell.isEmpty())
    {
        ui->check_speciyUserHome->setChecked(true);
        ui->edit_loginShell->setText(info.shell);
    }
    else
    {
        ui->check_speciyUserHome->setChecked(false);
    }
...
AdvanceSettingsInfo AdvanceSettings::getInfo ()
{
    ...
    if (ui->check_specifyUserID->isChecked())
    {
        info.uid = ui->edit_userID->text();
    }
    info.shell = ui->edit_loginShell->text();
    if (ui->check_speciyUserHome->isChecked())
    {
...
void AdvanceSettings::initUI ()
{
    ...
    connect(ui->check_specifyUserID, &QCheckBox::stateChanged, [this] (int state) {
        if (state > 0)
        {
    ...
    connect(ui->check_speciyUserHome, &QCheckBox::stateChanged, [this] (int state) {
        if (state > 0)
        {
