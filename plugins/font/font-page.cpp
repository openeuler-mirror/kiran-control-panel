...
void FontPage::initUI()
{
    ...
    KiranPushButton::setButtonType(ui->btn_delete, KiranPushButton::BUTTON_Normal);
    ui->btn_add->setIcon(QIcon::fromTheme("ksvg-add"));
...
void FontPage::onTableContextMenu(const QPoint &pos)
{
    ...
    QAction *disableAction = menu.addAction(QIcon::fromTheme("ksvg-disabled"), tr("Disable"));
    disableAction->setEnabled(hasEnabled);
    QAction *enableAction = menu.addAction(QIcon::fromTheme("ksvg-enabled"), tr("Enable"));
    enableAction->setEnabled(hasDisabled);
    QAction *deleteAction = menu.addAction(QIcon::fromTheme("ksvg-trash"), tr("Delete"));
    QAction *openAction = nullptr;
    if (!QStandardPaths::findExecutable(QStringLiteral("mate-font-viewer")).isEmpty())
    {
        menu.addSeparator();
        openAction = menu.addAction(QIcon::fromTheme("ksvg-kcp-appearance-font"),
