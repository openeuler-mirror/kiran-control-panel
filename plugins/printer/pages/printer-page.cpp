...
void PrinterPage::initUiStyle()
{
    ...
    KiranPushButton::setButtonType(ui->addPrinterButton, KiranPushButton::BUTTON_Default);
    KiranPushButton::setButtonType(ui->startServiceButton, KiranPushButton::BUTTON_Default);
    KiranPushButton::setButtonType(ui->retryButton, KiranPushButton::BUTTON_Normal);
    ...
    applyTheme();
    connect(DEFAULT_PALETTE(), &Palette::baseColorsChanged, this, &PrinterPage::applyTheme);
}

void PrinterPage::applyTheme()
{
    auto *palette = DEFAULT_PALETTE();
    const QColor border = palette->getColor(Palette::ACTIVE, Palette::BORDER);
    const QColor muted = palette->getColor(Palette::DISABLED, Palette::TEXT);

    QPalette descPal;
    descPal.setColor(QPalette::WindowText, muted);
    ui->errorDescLabel->setPalette(descPal);
