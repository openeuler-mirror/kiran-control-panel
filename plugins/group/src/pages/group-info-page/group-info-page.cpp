...
void GroupInfoPage::initUI()
{
    ...
    StylePropertyHelper::setButtonType(ui->btn_add_user, BUTTON_Default);
    StylePropertyHelper::setButtonType(ui->btn_delete, BUTTON_Warning);

    ui->btn_change_name->setIcon(QIcon(":/kcp-group-images/change_name_icon.svg"));
    ui->btn_change_name->setFocusPolicy(Qt::NoFocus);
    ui->btn_change_name->setStyleSheet("border:none;");

    // 用户组icon
    QImage iconImage(":/kcp-group-images/group_icon_large.png");
...
void GroupInfoPage::appendMemberListItem(const QString &userName)
{
    ...
    item->setRightBtnIcon(QIcon(":/kcp-group-images/delete_member_icon.svg"));
...
void GroupInfoPage::updateIcon()
{
    ...
    QIcon editIcon(":/kcp-group-images/change_name_icon.svg");
    QPixmap editPixmap = editIcon.pixmap(90, 90);
    ...
    if (Kiran::StylePalette::instance()->paletteType() != Kiran::PALETTE_DARK)
    {
        ...
        QImage editImage = editPixmap.toImage();
        editImage.invertPixels(QImage::InvertRgb);
        editPixmap = QPixmap::fromImage(editImage);
    }

    ui->avatar->setPixmap(groupPixmap);
    ui->btn_change_name->setIcon(editPixmap);
}