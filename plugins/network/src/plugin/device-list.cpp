...
void DeviceList::init(NetworkManager::Device::Type type)
{
    ...
    for (auto device : managedDeviceList)
    {
        ...
        {
            KLOG_DEBUG() << "wireless Device:" << wirelessDevice << " requestScan error:" << replyRequestScan.error();
            continue;
        }
        KLOG_DEBUG() << "wireless Device:" << wirelessDevice << " requestScan reply:" << replyRequestScan.reply();
    }
}
...
void DeviceList::addDevice(const QString &devicePath)
{
    ...
    if ( device.isNull() )
    {
        KLOG_ERROR() << "failed to add device, failed to find device:" << devicePath;
        return;
    }
    ...
    {
        KLOG_WARNING() << "device has been added:" << device->type() 
                                                   << device->interfaceName() 
                                                   << device->uni();
        return;
    }

    KLOG_INFO() << "add new device:" << device->type()
                                     << device->interfaceName() 
                                     << device->uni();
